...
class SequencerClient:
    ...
    def __init__(
        ...
        self.handle = ffi.new("snd_seq_t **", ffi.NULL)
        err = asound.snd_seq_open(self.handle, sequencer_name_b, streams, mode)
        _check_alsa_error(err)
        asound.snd_seq_set_client_name(self.handle[0], client_name_b)
        self.client_id = asound.snd_seq_client_id(self.handle[0])
    ...
    def close(self):
        if self.handle is None:
            return
        asound.snd_seq_close(self.handle[0])
    ...
    def create_queue(self, name: str = None) -> SequencerQueue:
        self._check_handle()
        if name is not None:
            queue = asound.snd_seq_alloc_named_queue(self.handle[0], name.encode("utf-8"))
        else:
            queue = asound.snd_seq_alloc_queue(self.handle[0])
        _check_alsa_error(queue)
        return SequencerQueue(self, queue)

    def drain_output(self):
        self._check_handle()
        err = asound.snd_seq_drain_output(self.handle[0])
        _check_alsa_error(err)

    def drop_output(self):
        self._check_handle()
        err = asound.snd_seq_drop_output(self.handle[0])
        _check_alsa_error(err)

    def event_input(self):
        self._check_handle()
        result = ffi.new("snd_seq_event_t**", ffi.NULL)
        err = asound.snd_seq_event_input(self.handle[0], result)
        _check_alsa_error(err)
    ...
    def event_output(self,
        ...
        err = asound.snd_seq_event_output(self.handle[0], alsa_event)
        _check_alsa_error(err)
